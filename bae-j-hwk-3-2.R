...
filter.data.1970_1990 <- tax_data %>%
  mutate(ln_sales = log(sales_per_capita), ln_price = log(price_cpi)) %>%
  filter(Year %in% 1970:1990)

ols1 <- lm(formula = ln_sales ~ ln_price, data = filter.data.1970_1990)
modelsummary(ols1)
...
iv1 <- feols(fml = ln_sales ~ 1 | ln_price ~ tax_cpi, data = filter.data.1970_1990)
modelsummary(iv1)


# 8. Show the first stage and reduced-form results from the instrument.

first_stage1 <- lm(formula = ln_price ~ tax_cpi, data = filter.data.1970_1990)
reduced_form1 <- lm(formula = ln_sales ~ tax_cpi, data = filter.data.1970_1990)

modelsummary(first_stage1)
modelsummary(reduced_form1)

# 9. Repeat the questions 6-8 focusing on the period from 1991 to 2015

filter.data.1991_2015 <- tax_data %>%
  mutate(ln_sales = log(sales_per_capita), ln_price = log(price_cpi)) %>%
  filter(Year %in% 1991:2015)

# 9-1 Regress log sales on log prices to estimate the price elasticity of demand over that period

ols2 <- lm(formula = ln_sales ~ ln_price, data = filter.data.1991_2015)
modelsummary(ols2)

# 9-2. Regress log sales on log prices using the total (federal and state) cigarette tax (in dollars) as an instrument for log prices.

iv2 <- feols(fml = ln_sales ~ 1 | ln_price ~ tax_cpi, data = filter.data.1991_2015)
modelsummary(iv2)

# 9-3. Show the first stage and reduced-form results from the instrument.

first_stage2 <- lm(formula = ln_price ~ tax_cpi, data = filter.data.1991_2015)
reduced_form2 <- lm(formula = ln_sales ~ tax_cpi, data = filter.data.1991_2015)
modelsummary(first_stage2)
...
#still working on the analysis, tables are almost done
